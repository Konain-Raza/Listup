...
const Dropdown = ({ selected, taskId, handleStatusChange, isViewable }) => {
  ...
  const [openDropdownId, setOpenDropdownId] = useState(null); // Local state to track the open dropdown
  ...
  const toggleDropdown = () => {
    if (openDropdownId === taskId) {
      setIsOpen(false); // Close the dropdown if it is already open
      setOpenDropdownId(null); // Set the ID to null when closing
    } else {
      setIsOpen(true); // Open the dropdown
      setOpenDropdownId(taskId); // Set this dropdown as the open one
    }
  };
  ...
  return (
    <div className="relative w-max h-max" ref={dropdownRef}>
      <div
        className={`w-[106px] text-base px-2 py-1 border-gray-300 dark:border-gray-500 rounded-md ${isViewable ? "cursor-default" : "cursor-pointer" }  ${
